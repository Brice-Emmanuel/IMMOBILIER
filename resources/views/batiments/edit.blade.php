...
<div class="max-w-lg mx-auto bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
    <div class="bg-[#0A2E38] text-white p-5 flex items-center justify-between">
        <h1 class="font-bold uppercase text-xs tracking-wider flex items-center">
            <i class="fa-solid fa-pen-to-square mr-2 text-[#C6E900]"></i> Modifier le Bâtiment
        </h1>
    ...
    <form method="POST" action="{{ route('batiments.update', $batiment) }}" class="p-6 space-y-4">
        @csrf 
        @method('PUT')

        <div>
            <label class="block text-xs font-bold uppercase text-gray-700 mb-1.5">Nom du Bâtiment <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="{{ old('name', $batiment->name) }}" required 
        ...
        <div>
            ...
            <input type="text" name="ville" value="{{ old('ville', $batiment->ville) }}" required 
        ...
        <div>
            ...
            <textarea name="adresse" rows="3" 
                      ...
                      class="w-full border border-gray-300 rounded-xl px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A2E38] transition">{{ old('adresse', $batiment->adresse) }}</textarea>
            @error('adresse') 
                <span class="text-red-500 text-xs mt-1 block"><i class="fa-solid fa-circle-exclamation mr-1"></i>{{ $message }}</span> 
            @enderror
        </div>
